  ...
  def main(args: Array[String]) {
    ...
    val trainingData = (0 to repeatDays).map(
      x =>{
        ...
        println(endTime)
        println(preTime)
        DT.getLabeledPoint(trainingSet,startTime,endTime,preTime)
      }
    ).reduce((x,y) => x.union(y))
    val trainingSetFiltered = trainingSet.groupBy(x =>x.itemId).join(itemList.map(x => (x, 0))).flatMap( x => x._2._1)
...
///不能广播不知道是什么bug
    ...
    val testData = DT.getLabeledPointWithUI(trainingSetFiltered,ft0,ft1,ft2)
